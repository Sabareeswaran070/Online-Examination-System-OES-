...
exports.generateAIQuestions = async (req, res, next) => {
  try {
    ...
    if (examId) {
      // College admin can add to any exam in their college
    ...
    logger.info(`AI generated ${savedQuestions.length} ${qType} questions by college admin ${req.user.email}`);
    ...
    logger.error('College Admin AI generation error:', error);
    res.status(500).json({
      success: false,
