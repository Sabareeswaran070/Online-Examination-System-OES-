...
const LANGUAGES = [
    ...
    { value: 'python', label: 'Python', color: 'text-blue-600 bg-primary-50' },
    { value: 'java', label: 'Java', color: 'text-red-600 bg-red-50' },
    { value: 'cpp', label: 'C++', color: 'text-purple-600 bg-primary-50' },
...
const CodingQuestionModal = ({
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center gap-4 p-3 bg-gray-50 rounded-xl border">
                ...
                <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-sm text-eyDark font-medium">{formData.hiddenTestCases.length} hidden</span>
                </div>
            </div>
            ...
            <div className="rounded-xl border-2 border-purple-200 overflow-hidden">
                ...
                <div className="p-4 space-y-4 bg-white">
                    {formData.hiddenTestCases.map((tc, idx) => (
                        <div key={idx} className="relative group border border-purple-200 rounded-xl p-4 bg-primary-50/30 hover:bg-primary-50/60 transition-colors">
                            <div className="flex items-center justify-between mb-3">
    ...
    const renderCodeTab = () => (
        <div className="space-y-6">
                    ...
                    {LANGUAGES.map(lang => (
                        <button
                            ...
                            className={`px-4 py-2.5 rounded-xl text-sm font-bold border-2 transition-all ${formData.programmingLanguage === lang.value
                                ? 'border-indigo-500 bg-primary-50 text-eyDark shadow-sm ring-2 ring-indigo-200'
            ...
            {/* Full solution logic (teacher reference) */}
            <div>
                <label className="block text-sm font-bold text-gray-700 mb-2 mt-6">
                    Full Reference Solution
                    <span className="text-xs font-normal text-purple-600 ml-2">(Internal only — students will NOT see this)</span>
                </label>
                <div className="rounded-xl overflow-hidden border-2 border-purple-200 shadow-sm">
                    <div className="bg-purple-900 px-4 py-2 flex items-center justify-between">
                        <span className="text-xs text-purple-200 font-mono flex items-center gap-1.5">
                            <FiCheckCircle className="w-3.5 h-3.5" /> Correct Implementation
                        </span>
                        <Badge variant="purple" className="text-[10px] bg-purple-800 text-purple-200 border-none">
                            Admin Only
                        </Badge>
                    </div>
                    <textarea
                        ...
                        placeholder="// Write the completely working solution code here..."
                        rows={10}
                        className="w-full px-4 py-4 text-sm font-mono bg-gray-900 text-purple-300 outline-none resize-none border-0"
                        spellCheck={false}
                    />
                </div>
            </div>
        ...
        <div className="space-y-6">
            ...
            <div className="p-5 bg-gray-50 rounded-xl border">
                <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2">
                    <FiCpu className="text-primary-600" /> Execution Limits
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            {[500, 1000, 2000, 5000].map(t => (
                                <button
                                    ...
                                    className={`px-2.5 py-1 rounded-md text-xs font-medium transition-colors ${Number(formData.timeLimit) === t
                                        ? 'bg-indigo-100 text-eyDark border border-indigo-300'
                    ...
                    <div>
                        <label className="flex items-center gap-2 text-sm font-semibold text-gray-700 mb-2">
                            <FiCpu className="text-primary-600" /> Memory Limit
                        </label>
                        <div className="flex items-center gap-3">
                            <input
                                type="number"
            ...
            {/* Summary Card */}
            <div className="p-5 bg-primary-50/50 border border-indigo-200 rounded-xl">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex gap-1.5">
                            {TABS.map((tab) => (
                                <div
                                    key={tab.id}
                                    className={`h-1.5 w-8 rounded-full transition-colors ${activeTab === tab.id ? 'bg-primary-500' : 'bg-gray-200'
                                        }`}
                                />
                            ))}
                        </div>
